...
def _get_cc_datasets(data, labels, crop_size, blur=False, color_jitter_s=0.5, data_val=None,
                     labels_val=None, transforms=None):
    if data_val is None:
        data_val, labels_val = data, labels
    train_dataset = ImageDataset(data, annotations=labels, transformations=transforms)
    val_dataset = ImageDataset(data_val, annotations=labels_val, transformations=transforms)
    return train_dataset, val_dataset


def imagenet_10_cc(dataset_path=None, crop_size=224, transforms=None, *args, **kwargs):
...
def imagenet_dogs_cc(dataset_path=None, crop_size=224, transforms=None, *args, **kwargs):
...
def imagenet(dataset_path=None, crop_size=224, transforms=None, *args, **kwargs):
    data, labels, _, _ = get_imagenet(dataset_path, "default")
    train_dataset, val_dataset = _get_cc_datasets(data, labels, crop_size, blur=True, color_jitter_s=1,
                                                  transforms=transforms)
    return train_dataset, val_dataset


def imagenet_tiny(dataset_path=None, crop_size=224, transforms=None, *args, **kwargs):
